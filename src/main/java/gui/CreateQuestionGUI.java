...
public class CreateQuestionGUI extends JFrame {
	...
	private List<XMLGregorianCalendar> datesWithEventsCurrentMonth = new Vector<XMLGregorianCalendar>();
	...
	private void jbInit(Vector<Event> v) throws Exception {
		...
		//gregory.setTime(jCalendar.getDate());

//		XMLGregorianCalendar dealCloseDate = DatatypeFactory.newInstance().newXMLGregorianCalendar(gregory);
		
		
		datesWithEventsCurrentMonth=facade.getEventsMonth(ApplicationLauncher.toXMLGregorianCalendar(jCalendar.getDate()));
...
//					public void propertyChange(PropertyChangeEvent propertychangeevent) {
				if (propertychangeevent.getPropertyName().equals("locale")) {
					...
					try {
						...
						List<Event> events = facade.getEvents(ApplicationLauncher.toXMLGregorianCalendar(firstDay));
...
public static void paintDaysWithEvents(JCalendar jCalendar,List<XMLGregorianCalendar> datesWithEventsCurrentMonth) {
		...
		if (Locale.getDefault().equals(new Locale("es")))
	 	...
	 	for (XMLGregorianCalendar d1:datesWithEventsCurrentMonth){
	 		
	 		calendar.setTime(d1.toGregorianCalendar().getTime());
